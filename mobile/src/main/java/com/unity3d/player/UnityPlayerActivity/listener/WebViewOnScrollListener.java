package com.unity3d.player.UnityPlayerActivity.listener;

import com.unity3d.player.UnityPlayerActivity.view.ScrollWebView;

public class WebViewOnScrollListener implements ScrollWebView.OnScrollListener {
	private static final int THRESHOLD = 100;

	private int mCounter = 0;
	private boolean mShown = true;

	@Override
	public void onScrollChanged(ScrollWebView scrollWebView, int x, int y, int oldx, int oldy) {
		int diff = y - oldy;

		if (y == 0) {
			if (!mShown) {
				mShown = true;
				mCounter = 0;
			}
		} else {
			if (mShown && mCounter > THRESHOLD) {
				mShown = false;
				mCounter = 0;
			} else if (!mShown && mCounter < -THRESHOLD) {
				mShown = true;
				mCounter = 0;
			}
		}

		if ((mShown && diff > 0) || (!mShown && diff < 0)) {
			mCounter += diff;
		}
	}
}
